...
data "archive_file" "lambda_apigateway_returns_json" {
    type        = "zip"
    source_file = "${path.root}/../src/lambda_apigateway_returns_json.py"
    output_path = "${path.root}/../.build/lambda_apigateway_returns_json.zip"
}
...
resource "aws_lambda_function" "apigateway_returns_json" {
    function_name = "apigateway_returns_json"

    description = "Lambda function triggered by API Gatewway and that returns a simple JSON"

    filename         = data.archive_file.lambda_apigateway_returns_json.output_path
    source_code_hash = data.archive_file.lambda_apigateway_returns_json.output_base64sha256

    layers = [ 
        aws_lambda_layer_version.awsmate.arn
    ]

    handler = "lambda_apigateway_returns_json.${var.lambda_entry_point}"
