...
This example API is accessible from the whole internet to anyone who would find its URL, so we recommend undeploying the example application after 
use (see :ref:`section "Undeployment" <Undeployment>` below). Its URL is generated by AWS at deployment time so there is no risk to see it memorized by anybody.
